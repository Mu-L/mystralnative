...
Platform-specific information for running MystralNative on Linux.
...
```
mystral-linux-x64-v8-wgpu.zip
```

**V8 + wgpu-native** provides the best Linux experience with Vulkan backend.
...
```bash
# Download
curl -L -o mystral.zip https://github.com/mystralengine/mystralnative/releases/latest/download/mystral-linux-x64-v8-wgpu.zip
...
|-----------|----------------|-------|
| V8 | wgpu-native | Recommended, Vulkan backend |
| QuickJS | wgpu-native | Smaller binary |

**Note**: Dawn builds are not officially supported on Linux yet due to build complexity.
...
MystralNative currently requires X11. On Wayland:
...
- [Running Games](/docs/guides/running-games) - CLI usage
- [Building from Source](/docs/guides/building) - Build options
