...
Build MystralNative from source for custom configurations or development.
...
```bash
# QuickJS (default) - smallest binary, no JIT
cmake -B build -DMYSTRAL_USE_QUICKJS=ON

# V8 - full ES2024+, JIT compilation
cmake -B build -DMYSTRAL_USE_V8=ON

# JavaScriptCore - macOS/iOS system engine
cmake -B build -DMYSTRAL_USE_JSC=ON
```
...
```bash
# wgpu-native (default) - Rust implementation, best iOS support
cmake -B build -DMYSTRAL_USE_WGPU=ON

# Dawn - Chrome's implementation, best shader compatibility
cmake -B build -DMYSTRAL_USE_DAWN=ON
```

### Combined Example

```bash
# V8 + Dawn (recommended for desktop)
cmake -B build \
    -DMYSTRAL_USE_V8=ON \
...
- [Configuration](/docs/guides/configuration) - Runtime configuration
- [Embedding](/docs/api/embedding) - Use as a library
